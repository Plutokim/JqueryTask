function autocompleteCountries(countries){
    let availableCountries = countries.map(element=> element.name);
    $( "#countries" ).autocomplete({
        source: availableCountries
      });
      $( "#countries" ).on( "autocompletesearch", function( event, ui ) {
        renderCountries(countries);
      } );

      $( "#countries" ).on( "autocompleteselect", function( event, ui ) {
          let a = ui.item.value;
          let selectedCountry = countries.find(item => item.name ==a);
          let selectedCountryArray = [selectedCountry];
          renderCountries(selectedCountryArray);
    } );
}
...
function renderCountries(countries) {
    ...
    autocompleteCountries(savedCountries);
}
